...
#if defined(POINTING_DEVICE_DEBUG)
    pd_dprintf("Made it to ==> pointing_device_init(void)\n");
#endif
    for (uint8_t i = 0; i < POINTING_DEVICE_COUNT; i++) {
#if defined(SPLIT_KEYBOARD)
#    if defined(POINTING_DEVICE_DEBUG)
        pd_dprintf("pointing_device_init ==> SPLIT_KEYBOARD is defined\n");
#    endif
...
#else
#    if defined(POINTING_DEVICE_DEBUG)
        pd_dprintf("pointing_device_init ==> SPLIT_KEYBOARD is NOT defined\n");
#   endif
#endif
...
bool pointing_device_task_get_pointing_reports(report_mouse_t* report) {
#    if defined(POINTING_DEVICE_DEBUG)
         pd_dprintf("pointing_device_task_get_pointing_reports ==> Made it. \n");
#    endif
...
#if defined(SPLIT_KEYBOARD)
#    if defined(POINTING_DEVICE_DEBUG)
         pd_dprintf("pointing_device_task_get_pointing_reports ==> SPLIT_KEYBOARD is enabled\n");
         if (POINTING_DEVICE_THIS_SIDE(i)) {
                pd_dprintf("pointing_device_task_get_pointing_reports ==> SPLIT_KEYBOARD ==> do NOT skip processing devices on this side\n");
            }
#    endif
        if (!POINTING_DEVICE_THIS_SIDE(i)) {
#           if defined(POINTING_DEVICE_DEBUG)
              pd_dprintf("pointing_device_task_get_pointing_reports ==> SPLIT_KEYBOARD ==> skip processing devices not on this side\n");
#           endif
...
#   if defined(POINTING_DEVICE_DEBUG)
        pd_dprintf("pointing_device_task_get_pointing_reports ==> device_was_ready == %d\n", device_was_ready);
#   endif
    return device_was_ready;
}

__attribute__((weak)) bool pointing_device_task(void) {
    bool device_was_ready    = pointing_device_task_get_pointing_reports(&local_report);
...
#    if defined(POINTING_DEVICE_DEBUG)
         pd_dprintf("pointing_device_task ==> local_report: local_report.x: %d,local_report.y: %d\n", local_report.x, local_report.y);
#    endif
